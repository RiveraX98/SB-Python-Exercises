...
class currrency_convert_test(TestCase):
    def test_check_info(self):
        ...
        self.assertEqual(check_info("USD", "ZZZ", "500"), "Invalid currency code")
    ...
    def test_verify_form_info(self):
        with app.test_client() as client:
            res = client.post("/verify_form", data={"from": "USD", "into": "JPY", "amount": "500"})
            self.assertEqual(res.status_code, 302)
            self.assertEqual(res.location, "/convert")
            
            res = client.post("/verify_form", data={"from": "USD", "into": "ZZZ", "amount": "500"})
